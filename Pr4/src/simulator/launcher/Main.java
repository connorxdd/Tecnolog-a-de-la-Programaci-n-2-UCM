...
public class Main {
	...
	/*
	 * Pues System.out es un PrintStream, con lo que deberías poder guardarlo en
	 * un OutputStream OutputStream algo= System.out
	 * 
	 * Para luego poder escribir, te recomiendo que crees un new
	 * PrintStream(algo); y con el uses el println.
	 */
	...
	private static void parseGravityLawsOption(CommandLine line) throws ParseException {
		...
		// this line is just a work around to make it work even when
		// _gravityLawsFactory
	...
	private static void startGUIMode() throws InvocationTargetException, InterruptedException {
		...
		PhysicsSimulator simulator = new PhysicsSimulator(_dtime, gravity);
		Controller control = new Controller(simulator, Main._bodyFactory, Main._gravityLawsFactory);
		ControlPanel cp = new ControlPanel(control);
		cp.run_sim(_step);		
	}
